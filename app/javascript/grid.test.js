  ...
  it('should render a valid game from the grid', async () => {
    ...
    await new Promise((resolve, reject) => {
      jsdom.env(html, [], (err, window) => {
        BlockGrid.prototype.render.apply(testGrid, [
          window.document.querySelector('#gridEl')
        ]);
        resolve();
      });
    });
    ...
    assert.equal(
      firstCol.children[0].style.background,
      'red',
      'rendered game tile has correct colour'
    );
  });

  it('should remove adjacent elements of the same colour from the grid when an element is clicked', () => {
    ...
    assert.deepEqual(
      testGrid.grid[0][0],
      new Block(COLOURS[0]),
      'column 1 block 1 is red'
    );
    assert.deepEqual(
      testGrid.grid[0][1],
      new Block(COLOURS[0]),
      'column 1 block 2 is red'
    );
    assert.deepEqual(
      testGrid.grid[2][0],
      new Block(COLOURS[0]),
      'column 3 block 1 is red'
    );
    assert.deepEqual(
      testGrid.grid[2][1],
      new Block(COLOURS[0]),
      'column 3 block 2 is red'
    );
  });
